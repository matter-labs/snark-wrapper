...
pub(crate) fn binary_select<E: Engine, CS: ConstraintSystem<E>>(
    cs: &mut CS,
    elements: &[GoldilocksField<E>],
    bits: &[Boolean],
) -> Result<GoldilocksField<E>, SynthesisError> {
...
pub(crate) fn materialize_powers_serial<
    E: Engine,
    CS: ConstraintSystem<E> + 'static,
>(
    cs: &mut CS,
    base: GoldilocksExtAsFieldWrapper<E, CS>,
    size: usize,
) -> Vec<GoldilocksExtAsFieldWrapper<E, CS>> {
    if size == 0 {
        return Vec::new();
    }
