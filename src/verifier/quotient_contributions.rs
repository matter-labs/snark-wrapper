...
>(
    ...
    let non_residues_for_copy_permutation = non_residues_for_copy_permutation::<GL, Global>(
        fixed_parameters.domain_size as usize,
        constants.num_variable_polys,
    );

    let non_residues_for_copy_permutation: Vec<_> = non_residues_for_copy_permutation
        .into_iter()
        .map(|el| GoldilocksAsFieldWrapper::constant(el, cs))
        .collect();
    ...
    let variables_polys_values: Vec<_> =
        (&mut source_it).take(constants.num_variable_polys).copied().collect();
    let witness_polys_values: Vec<_> =
        (&mut source_it).take(constants.num_witness_polys).copied().collect();
    // normal setup
    let constant_poly_values: Vec<_> =
        (&mut source_it).take(constants.num_constant_polys).copied().collect();
    let sigmas_values: Vec<_> = (&mut source_it)
        .take(constants.num_copy_permutation_polys)
        .copied()
        .collect();
    let copy_permutation_z_at_z = *source_it.next().unwrap();
    let grand_product_intermediate_polys: Vec<_> = (&mut source_it)
        .take(constants.num_intermediate_partial_product_relations)
        .copied()
        .collect();
    // lookup if exists
    let multiplicities_polys_values: Vec<_> = (&mut source_it)
        .take(constants.num_multiplicities_polys)
        .copied()
        .collect();
    let lookup_witness_encoding_polys_values: Vec<_> = (&mut source_it)
        .take(constants.num_lookup_subarguments)
        .copied()
        .collect();
    ...
    let lookup_tables_columns: Vec<_> = (&mut source_it)
        .take(constants.num_lookup_table_setup_polys)
        .copied()
        .collect();
    // quotient
    ...
    for (gate_idx, evaluator) in verifier
    ...
    let constants_for_gates_over_general_purpose_columns = fixed_parameters
        ...
        + verifier.parameters.num_constant_columns;
    ...
    check_specialized_gates_contribution(
        cs, 
        challenges, 
        &mut t_accumulator, 
        &src, 
    ...
    for el in quotient_chunks.into_iter() {
        GoldilocksExtAsFieldWrapper::<E, CS>::mul_and_accumulate_into(
            &mut t_from_chunks,
            &el,
            &pow,
            cs,
...
pub(crate) fn check_lookup_contribution<
    E: Engine, 
    CS: ConstraintSystem<E> + 'static,
>(
    cs: &mut CS,
    ...
    if verifier.lookup_parameters != LookupParameters::NoLookup {
        ...
        let lookup_witness_encoding_polys_polys_at_0 =
            &evaluations.all_values_at_0[..constants.num_lookup_subarguments];
        ...
        match verifier.lookup_parameters {
            ...
            | LookupParameters::TableIdAsConstant {
                ...
                let column_elements_per_subargument =
                    verifier.lookup_parameters.columns_per_subargument() as usize;
                assert!(
                ...
                let variables_columns_for_lookup = &variables_polys_values
                    [..(column_elements_per_subargument * constants.num_lookup_subarguments)];
                assert_eq!(
            ...
            | LookupParameters::UseSpecializedColumnsWithTableIdAsVariable {
                ...
                let column_elements_per_subargument =
                    verifier.lookup_parameters.specialized_columns_per_subargument() as usize;
                assert!(
                    fixed_parameters.table_ids_column_idxes.len() == 0
                        || fixed_parameters.table_ids_column_idxes.len() == 1
                );
                ...
                // first A polys
                let variables_columns_for_lookup = &variables_polys_values[verifier
                    .parameters
                    .num_columns_under_copy_permutation
                    ..(verifier.parameters.num_columns_under_copy_permutation
                        + column_elements_per_subargument * constants.num_lookup_subarguments)];
                assert_eq!(
                    lookup_witness_encoding_polys_values.len(),
                    variables_columns_for_lookup
...
pub(crate) fn check_specialized_gates_contribution<
    E: Engine, 
    CS: ConstraintSystem<E> + 'static,
>(
    cs: &mut CS,
    ...
    let specialized_evaluators_challenges =
        &challenges.pregenerated_challenges_for_gates_over_specialized_columns;
        ...
        .enumerate()
    {
        ...
        let (initial_offset, per_repetition_offset, total_constants_available) =
            verifier.offsets_for_specialized_evaluators[idx];
    ...
    assert_eq!(
        challenges_offset,
        constants.total_num_gate_terms_for_specialized_columns
    );

    Ok(())
}

pub(crate) fn check_general_purpose_gates_contribution<
    E: Engine, 
    CS: ConstraintSystem<E> + 'static,
>(
    cs: &mut CS,
    ...
    let general_purpose_challenges =
        &challenges.pregenerated_challenges_for_gates_over_general_purpose_columns;
    ...
    assert_eq!(
        challenges_offset,
        constants.total_num_gate_terms_for_general_purpose_columns
    );

    Ok(())
}

pub(crate) fn check_copy_permutation_contribution<
    E: Engine, 
    CS: ConstraintSystem<E> + 'static,
>(
    cs: &mut CS,
