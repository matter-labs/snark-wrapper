...
>(
    ...
    for queries in proof.queries_per_fri_repetition.iter() {
        let query_index_lsb_first_bits =
            bools_buffer.get_bits(cs, transcript, max_needed_bits)?;
        ...
        for (idx, (interpolation_degree_log2, fri_query)) in constants.fri_folding_schedule
            .iter()
            ...
            .enumerate()
        {
            ...
            for challenge in challenges.iter() {
                for (i, [a, b]) in elements_to_interpolate.array_chunks::<2>().enumerate() {
                    ...
                    GoldilocksExtAsFieldWrapper::<E, CS>::mul_by_base_and_accumulate_into(
                        &mut result,
                        &pow,
                        &diff,
                        cs,
...
>(
    ...
    let leaf_hash = <H as CircuitGLTreeHasher<E>>::hash_into_leaf(
        cs,
        leaf_elements,
    )?;
    verify_proof_over_cap::<E, H, CS>(
        cs,
        proof,
        tree_cap,
        &leaf_hash,
...
fn pow_from_precomputations<E: Engine, CS: ConstraintSystem<E> + 'static>(
    cs: &mut CS,
    bases: &[GoldilocksAsFieldWrapper<E, CS>],
    bits: &[Boolean],
) -> GoldilocksAsFieldWrapper<E, CS> {
...
>(
    ...
    let mut denom =
        GoldilocksExtAsFieldWrapper::<E, CS>::from_wrapper_coeffs_in_base([domain_element, zero_base]);
