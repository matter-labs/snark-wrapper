...
> AllocatedSingleRoundQueries<E, H> {
...
impl<
    ...
    HS: TreeHasher<GL, Output = E::Fr>,
    H: CircuitGLTreeHasher<E, CircuitOutput = Num<E>, NonCircuitSimulator = HS>    
> AllocatedOracleQuery<E, H> {
    pub fn allocate_from_witness<CS: ConstraintSystem<E>>(
        cs: &mut CS,
        witness: Option<OracleQuery<GL, H::NonCircuitSimulator>>,
        leaf_size: usize,
        proof_depth: usize,
    ) -> Result<Self, SynthesisError> {
        let num_elements = leaf_size;
        let leaf_elements = witness.as_ref().map(|el| el.leaf_elements.iter().copied());
        let leaf_elements = allocate_num_elements(
            cs, 
            num_elements, 
            leaf_elements, 
            GoldilocksField::alloc_from_field
        )?;

        let num_elements = proof_depth;
        let proof = witness.as_ref().map(|el| el.proof.iter().cloned());
        let proof = allocate_num_elements(
            cs, 
            num_elements, 
            proof, 
            Num::alloc
