...
pub struct CircuitPoseidon2Sponge<
    E: Engine,
    const RATE: usize,
    const WIDTH: usize,
...
impl<
    ...
    const CHUNK_BY: usize,
    const ABSORB_BY_REPLACEMENT: bool,
> CircuitPoseidon2Sponge<E, RATE, WIDTH, CHUNK_BY, ABSORB_BY_REPLACEMENT> {
    pub fn new() -> Self {
    ...
    pub fn try_get_commitment<CS: ConstraintSystem<E>>(
        &self, 
        cs: &mut CS
    ) -> Result<Option<[Num<E>; RATE]>, SynthesisError> {
        if self.filled != 0 {
            return Ok(None);
        }

        let mut result = [Num::zero(); RATE];
        for (dst, src) in result.iter_mut()
            .zip(self.state.iter())
        {
    ...
    pub fn absorb_single_gl<CS: ConstraintSystem<E>>(
        &mut self,
        cs: &mut CS,
    ...
    pub fn absorb<T: Into<LinearCombination<E>> + Clone, CS: ConstraintSystem<E>>(
        &mut self, 
        cs: &mut CS,
        values: &[T]
    ) -> Result<(), SynthesisError> {
        ...
        if len + pos < RATE {
            for (dst, src) in self.buffer[pos..pos+len].iter_mut()
                .zip(values.iter())
            {
                *dst = src.clone().into();
            }
        ...
        for (dst, src) in self.buffer[..new_pos].iter_mut()
            .zip(values[chunk_finish..].iter())
        {
            *dst = src.clone().into();
        }
    ...
    pub fn finalize_reset<CS: ConstraintSystem<E>>(
        &mut self, 
        cs: &mut CS,
    ) -> Result<[Num<E>; RATE], SynthesisError> {
        // padding
        ...
        let mut state = std::mem::replace(
            &mut self.state, 
            [(); WIDTH].map(|_| LinearCombination::zero()));
